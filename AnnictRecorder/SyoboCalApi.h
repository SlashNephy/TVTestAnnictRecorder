...
namespace SyoboCal {
    ...
    static std::vector <LookupProgramResult>
    LookupProgram(const SYSTEMTIME &start, const DWORD seconds, const int chId) {
        const auto stTime = std::format("{:04d}{:02d}{:02d}_{:02d}{:02d}{:02d}-", start.wYear, start.wMonth, start.wDay,
        ...
        const auto range = std::format("{}{:04d}{:02d}{:02d}_{:02d}{:02d}{:02d}", stTime, end.tm_year + 1900,
        ...
        for (auto &item: items) {
            ...
            const auto stSubTitle = rawSTSubTitle.empty() ? std::nullopt : std::optional(
                    std::string(rawSTSubTitle.child_value()));
            ...
            PrintDebug(L"TID = {}, Count = {} ~ {}, SubTitle = {}", titleId, countStart, countEnd,
                       stSubTitle.has_value());
