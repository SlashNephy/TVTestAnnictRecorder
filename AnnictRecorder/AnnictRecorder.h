...
namespace AnnictRecorder {
    ...
    static void UpdateWorkStatus(
        ...
        if (isLastEpisode && Config.SetWatchedInLastEpisode) {
            newStatus = "watched";
        } else if ((isFirstEpisode && Config.SetWatchingStatusInFirstEpisode) ||
                   (!isLastEpisode && Config.SetWatchingStatusInAnyEpisodes)) {
            newStatus = "watching";
        }
        ...
        if (newStatus.has_value()) {
            try {
                ...
                if (status != newStatus.value()) {
                    ...
                    PrintDebug(L"作品のステータスを「{}」→「{}」に変更しました。(WorkID={})", Multi2Wide(status),
                               Multi2Wide(newStatus.value()), annictWorkId);
                }
            }
            catch (...) {
            }
    ...
    static std::optional <Annict::Episode> FindEpisode(
        ...
        try {
            ...
            const auto episodeIterator = std::ranges::find_if(episodes, [episodeCount, episodeTitle](
                    const Annict::Episode &episode) {
                // 話数が一致
                if (episode.number.has_value() &&
                    episodeCount == episode.number.value())  // NOLINT(clang-diagnostic-float-equal)
                {
                    return true;
                }
                ...
                if (const auto numberText = std::format("第{:.0f}話", episodeCount); episode.numberText.has_value() &&
                                                                                     numberText ==
                                                                                     episode.numberText.value()) {
                    return true;
                }

                if (const auto numberText = std::format("#{:.0f}", episodeCount); episode.numberText.has_value() &&
                                                                                  numberText ==
                                                                                  episode.numberText.value()) {
                    return true;
                }
                ...
                // サブタイトルが一致
                return episodeTitle.has_value() && episode.title.has_value() &&
                       episodeTitle.value() == episode.title.value();
            });
    ...
    static CreateRecordResult CreateEpisodeRecord(
        ...
        if (!episode.has_value()) {
            PrintDebug(L"Annict でのエピソードデータが見つかりませんでした。スキップします。(WorkID={}, Count={})",
                       annictWorkId, episodeCount);
    ...
    static std::vector <CreateRecordResult> CreateRecord(
        ...
        if (!ChannelDefinition.has_value()) {
            PrintDebug(
                    L"saya のチャンネル定義に存在しないチャンネルです。スキップします。(サービス名: {}, サービス ID: {})",
                    Service.szServiceName, Service.ServiceID);
        ...
        if (ChannelDefinition.value().syobocalId.has_value()) {
            ...
            try {
                if (const auto syoboCalPrograms = SyoboCal::LookupProgram(Program.StartTime, Program.Duration,
                                                                          syoboCalChId); !syoboCalPrograms.empty()) {
                        ...
                        if (AnnictIds.contains(syoboCalTID)) {
                            ...
                            try {
                                ...
                                results.push_back(
                                        CreateRecordEach(
                                                ...
                                                isFirstEpisode || syoboCalProgram.isFirstEpisode ||
                                                syoboCalProgram.countStart <= 1,
                            ...
                            try {
                                ...
                                if (title.has_value()) {
                                    ...
                                    if (const auto annictWork = Annict::GetWorkByTitle(workTitle,
                                                                                       Config.AnnictToken); annictWork.has_value()) {
                                        results.push_back(
                                                CreateRecordEach(
                                                        annictWork.value(),
                                                        syoboCalProgram.countStart,
        ...
        if (ChannelDefinition.value().annictId.has_value()) {
            ...
            try {
                if (const auto annictPrograms = Annict::GetMyPrograms(annictChId, utcStartTime, Program.Duration,
                                                                      Config.AnnictToken); !annictPrograms.empty()) {
                    auto results = std::vector < std::vector < CreateRecordResult >> {};
                    for (const auto &annictProgram: annictPrograms) {
                        ...
                        results.push_back(
                                CreateRecordEach(
                                        ...
                                        isFirstEpisode || annictProgram.episode.number.value() <=
                                                          1, // NOLINT(clang-diagnostic-float-equal)
                                        isLastEpisode,
        ...
        if (Service.ServiceID == 333) {
            ...
            PrintDebug(L"抽出されたタイトル情報: Title={}, Count={} ~ {}", extraction.title, extraction.countStart,
                       extraction.countEnd);
        ...
        PrintDebug(L"しょぼいカレンダー / Annict に放送時刻が登録されていません。スキップします。(Service={}, SID={})",
                   Service.szServiceName, Service.ServiceID);
