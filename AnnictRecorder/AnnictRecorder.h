...
namespace AnnictRecorder
{
    ...
    static CreateRecordResult CreateEpisodeRecord(const Annict::Episode &episode, const Config &Config)
    {
        ...
        {
            workTitle = std::format(L"「{}」", Multi2Wide(episode.title.value()));
        }
        else
        {
            workTitle = std::format(L"「{}」", Multi2Wide(episode.work.title));
        }
        ...
        const Config &Config)
    {
        try
        {
            ...
            const auto episodeIterator = std::ranges::find_if(episodes, [episodeCount, episodeTitle](const Annict::Episode &episode)
                ...
                auto numberText = std::format("第{:.0f}話", episodeCount);
                if (episode.numberText.has_value() && numberText == episode.numberText.value())
                {
                    return true;
                }

                numberText = std::format("#{:.0f}", episodeCount);
                if (episode.numberText.has_value() && numberText == episode.numberText.value())
                {
                    return true;
                }
    ...
    static CreateRecordResult CreateWorkRecord(const Annict::Work &work, const Config &Config)
    {
        if (!Config.RecordDryRun)
        {
            try
        ...
        const YAML::Node &SayaDefinitions)
    {
        ...
        if (!ChannelDefinition.has_value())
        {
            ...
            return {
                {false, L"sayaのチャンネル定義に存在しないチャンネルです。"},
            };
        }
                ...
                if (const auto syoboCalPrograms = SyoboCal::LookupProgram(Program.StartTime, Program.Duration, syoboCalChId); !syoboCalPrograms.empty())
                {
                    ...
                    for (const auto &syoboCalProgram : syoboCalPrograms)
                    {
                        ...
                        else
                        {
                            ...
                            try
                            {
                                if (const auto title = SyoboCal::LookupTitle(syoboCalTID); title.has_value())
                                {
        ...
        PrintDebug(L"しょぼいカレンダー / Annict に放送時刻が登録されていません。スキップします。(SID={})", Program.ServiceID);

        return {
            {false, L"放送時刻データがありません。"},
        };
    }
}
